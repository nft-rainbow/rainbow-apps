  ...
  return (
    <div className="px-[48px] pt-[42px] flex flex-col justify-start">
      <div className="relative w-[654px] h-[654px]">
        <Suspense fallback={<PoapImgSuspenseFallback />}>
          <PoapImg />
        </Suspense>
      </div>
      ...
      <div className="mt-[12px] flex flex-row items-center text-[#696679]">
        <Suspense fallback={<p className="text-[24px] leading-[32px]">...</p>}>
          <ContractAddress />
        </Suspense>
      </div>

      <Suspense fallback={<ClaimAreaSuspenseFallback />}>
        <ClaimArea />
      </Suspense>

      <div className="flex flex-col items-center">
        <Suspense fallback={<ButtonAreaSuspenseFallback />}>
...
const PoapImgSuspenseFallback: React.FC = () => {
  return (
    <>
...
const MintCount: React.FC = () => {
  const activityId = useActivityId()!;
  const poapConf = usePoapConfig(activityId);

  return <>{poapConf?.mintedCount}</>;
};
  ...
  return (
    <>
      ...
      <p className="mt-[32px] text-[28px] leading-[32px] text-[#37334C] align-middle">
        可领取 <span className="text-[#6953EF] font-medium mx-[2px]">{!!poapConf && poapConf.count > 0 ? poapConf.count : 0}</span> 次
      </p>
    </>
  );
};

const ClaimAreaSuspenseFallback: React.FC = () => {
  return (
    <>
      <p className="mt-[42px] text-[40px] leading-[48px] font-semibold text-[#05001F]">...</p>
      <p className="mt-[24px] text-[28px] text-[#696679] leading-[36px]" dangerouslySetInnerHTML={{ __html: '...' }}></p>
      <p className="mt-[24px] text-[24px] leading-[32px] text-[#696679]">开始时间: ...</p>
      <p className="text-[24px] leading-[32px] text-[#696679]">
        结束时间: <span>...</span>
      </p>
      <p className="mt-[32px] text-[28px] leading-[32px] text-[#37334C] align-middle">
        可领取 <span className="text-[#6953EF] font-medium mx-[2px]">...</span> 次
      </p>
    </>
  );
};

const ButtonAreaSuspenseFallback: React.FC = () => {
  return (
    <>
      <button
        className={cx(
      ...
      <button className="mt-[24px] flex justify-center items-center h-[104px] w-[654px] border border-[#6953EF] rounded-[8px] text-[32px] font-medium leading-[40px] text-[#6953EF] opacity-30 pointer-events-none">
        邀请好友帮忙
      </button>
    </>
  );
